...
const ProductProp = (props) => {
  ...
  if (props.userRole === "ANON" || props.userRole === "USER") {
    return (
      <Card style={styles}>
        <a href={"/" + props.product.type + "s/" + props.product.name}>
          <Card.Img
            onMouseOver={(e) => {
      ...
      <Card style={styles}>
        ...
        <Card.Body>
          ...
          <Button
            ...
            variant="warning"
            onClick={() => {
              props.editProduct(props.product.name);
            }}
          >
            {" "}
            Edit{" "}
...
export default class ProductPage extends React.Component {
  ...
  editProduct(productName) {
    this.props.history.push("/" + this.props.type + "/edit/" + productName);
  }

  deleteProduct(product) {
    axios
      .delete("http://localhost:8080/product/delete/" + product.name)
      .then((res) => {
        // Refresh page
      ...
      .then((res) => {
        ...
        let array =
          this.props.type === "beauty"
            ? Array.from(res.data)
            : Array.from(res.data).filter(
                (element) => element.type === this.props.type
              );
        this.setState({
          products: array,
        });
